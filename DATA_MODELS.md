...
Fizika məzmunu `src/data/physicsContent.ts`, `physicsLessonDetails.ts` və `physicsLabDetails.ts` daxilində statik saxlanır. `PhysicsTopic` (`id`, `title`, `outline`, istəyə görə `sourceFile`, `pdfUrl`, `presentationNumber`, `explanations`, `checkQuestions`) LMS mövzusunu və dərs mətnini; `PhysicsLab` (`id`, `title`, istəyə görə `sourceFile`, `pdfUrl`, `objective`, `equipment`, `steps`, `result`, `theory`, `detailedSteps`, `calculations`) laboratoriya bələdçisini təsvir edir. `PhysicsExplanation` başlıq, mətn və istəyə görə düsturdan ibarətdir. `pdfUrl` Novcept-in mövcud Cloudflare R2 ictimai CDN ünvanına aparır; obyektlər `aztu/physics/` prefiksindədir. Bu seçimlər üçün yeni `localStorage` açarı və ya verilənlər bazası cədvəli yoxdur. Saytın nümayiş materialı tipi `SharedMaterial.type` üçün `docx` və `pptx` dəyərləri dəstəklənir.
