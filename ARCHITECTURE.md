...
Müəllim sənədləri LibreOffice ilə PDF-ə çevrilərək Novcept-in mövcud Cloudflare R2 bucket-ində `aztu/physics/` prefiksi ilə saxlanır. `physicsContent.ts` mövzu və laboratoriyanı uyğun `pdfUrl` ilə bağlayır; açılan panel `PhysicsPdfViewer` komponentini yalnız lazım olanda yükləyir. Bu komponent PDF.js ilə R2 faylını səhifə-səhifə canvas-da göstərir; ayrıca pəncərəyə keçid də var. Konspektin geniş hissələri klient tərəfində accordion vəziyyəti ilə tək-tək göstərilir. R2 ictimai CDN ünvanları server autentifikasiyası ilə qorunmur; bu, mövcud klient əsaslı girişdən ayrıca məhdudiyyətdir.
